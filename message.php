...
if($conn)
{
    ...
    if(isset($_POST['submit'])&&$_SERVER["REQUEST_METHOD"] == "POST" || isset($_FILES["image"]))
    {
            ...
            $namess=mysqli_real_escape_string($conn,$_POST['mestoreg']);
            $m=mysqli_query($conn,"SELECT REGNO FROM usertb WHERE NAMES='$namess'");
            ...
            {
            $sql = "INSERT INTO messages(mesfromreg,mestoreg,message,timestamp) VALUES ('$regno','$regno1','$messages',now())";
                if ($conn->query($sql) === TRUE) {
                    $error="Message is send!";
                } else {
...
<body>
    ...
    <nav>
        ...
        <a id="now" href="message.php"><i class="fa fa-comment" aria-hidden="true"></i></a>
    ...
    <div class="main2">
        ...
        <a href="message.php"><div class="home now1">Messages</div></a>
    ...
    <main>
        ...
        <div class="post-form">
            <form action="<?php htmlspecialchars($_SERVER['PHP_SELF'])?>" class="form" method="post" enctype="multipart/form-data">
                <p><?php echo $error;?></p>
            <h3 align="left">Send message:</h3>
            <h2> From<img src="pic/1.png" id="image" ><?php echo $login_session;?>  <sub>Verfied</sub></h2>
            <h2> To<img src="pic/1.png" id="image" >
            <select name="mestoreg" class="button">
            <?php
            $rest=mysqli_query($conn,"SELECT NAMES from usertb");
            $i=0;
           ...
           <input type="hidden" value="<?php echo $messages;?>" id="messageInput">
                <textarea name="message" rows="4" placeholder="type your message..."></textarea>
                <br>
           <input class="button" onclick="sendMessage()" align="right" name="submit" type="submit" value="Send message">
            </form>
            <script>
        ...
        <div class="posts">
            ...
            <?php
            $w=mysqli_query($conn,"SELECT * from messages where `mesfromreg`='$regno'");
            if($w){
